...
const DOWNSTREAM_SCRIPT_EXCLUDES = new Set(["bootstrap", "bootstrap:project", "bootstrap:neighbor"]);
...
function runBootstrapProject({
  ...
  const bootstrapScript = path.resolve(targetRoot, "tools/bootstrap/bootstrap-project.mjs");
  if (!fs.existsSync(bootstrapScript)) {
    fail("Missing target bootstrap script after managed copy: tools/bootstrap/bootstrap-project.mjs");
  }

  const templateLocalPath = normalizePath(path.relative(targetRoot, templateRoot));

  const args = [
    "tools/bootstrap/bootstrap-project.mjs",
    "--project-id",
    projectId,
    "--repo-owner",
    repoOwner,
...
function main() {
  ...
  const explicitWorkfilesPath =
    toNonEmptyString(args.agentsWorkfilesPath) ?? toNonEmptyString(args.agentsRootBase);
  ...
  const managedManifestTemplatePath = path.resolve(
    templateRoot,
    "tools/bootstrap/managed-files.template.json",
  );
  if (!fs.existsSync(managedManifestTemplatePath)) {
    fail("Template manifest missing: tools/bootstrap/managed-files.template.json");
  }
  ...
  ensureDir(path.resolve(targetRoot, ".agent-overrides"));
