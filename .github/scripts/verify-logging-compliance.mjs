...
const BASELINE_PATH = ".github/policies/logging-compliance-baseline.json";
...
function readBaseline() {
  if (!fs.existsSync(BASELINE_PATH)) {
    throw new Error(
      `Logging baseline missing at ${BASELINE_PATH}. Run: node .github/scripts/verify-logging-compliance.mjs --write-baseline`,
    );
  }
...
function writeBaseline(findings) {
  const payload = {
    metadata: {
      id: "agents-template-logging-compliance-baseline",
...
function checkAgainstBaseline(currentFindings, baseline) {
  ...
  console.error(
    "Run node .github/scripts/verify-logging-compliance.mjs --write-baseline after intentional migrations.",
  );
  process.exit(1);
}
